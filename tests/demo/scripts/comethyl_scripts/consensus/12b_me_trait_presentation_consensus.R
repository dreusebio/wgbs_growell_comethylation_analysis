...
step_dir      <- file.path(pipeline_root, "12b_me_trait_presentation", opt$adjustment_version)
cross_dir     <- file.path(step_dir, "cross_dataset")
...
for (set_file in set_files) {
  ...
  for (ds in dataset_inputs) {
    ...
    ds_label   <- ds$label
    ds_set_dir <- file.path(step_dir, ds_label, set_name)
